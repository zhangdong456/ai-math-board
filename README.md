...
- **25 个内置模板**（无需 API Key 即可体验）：
  - 数学 ×14：二次/一次/三角函数、圆、矩形/三角形/平行四边形面积演示、圆面积与割圆术、指数函数、导数几何意义（割线逼近切线）、高斯钟形曲线（正态分布）、三次函数
  - 物理 ×7：F=ma、匀变速直线运动（v-t 图面积=位移）、自由落体、欧姆定律、平抛运动（含月球重力）、阻尼振动（振幅指数衰减 + 包络线）
  - 化学 ×4：玻尔原子模型、氢氧燃烧配平（原子守恒）、分子 3D 球棍模型（CH₄/H₂O/CO₂/NH₃，纯 SVG 手写 3D 投影）
- **动画自动播放**：演示窗口内置「▶ 动画演示」，参数自动往返扫描或时间自动推进（16 种渲染器），拖滑块即停
- **我的模板库**：AI 生成的演示可一键「☆ 存为模板」，按学科归档存本地，下次从模板列表直接打开，可删除
- **坐标系随窗口缩放**：窗口放大，坐标系/曲线/刻度同比放大（SVG viewBox 自适应）
- **深空科幻 UI**：极光背景、霓虹辉光窗口、入场与悬停动效，支持 prefers-reduced-motion
- **知识验证层**：schema 校验 + 学科规则库 + 置信度门槛，AI 输出不可靠时拒绝生成并提示
- **白板窗口系统**：多窗口拖动/八向缩放/最大最小化/置顶，一键平铺/级联智能排列
- **画笔标注**：画笔/矩形/圆形/箭头/橡皮/撤销重做，覆盖整个白板
- **模型配置**：内置 DeepSeek / Kimi / GLM / 通义千问（阿里百炼）等厂商预设，自动填 BaseURL、视觉能力标注、测试连接、拉取模型列表；也支持任意 OpenAI 兼容接口
...
src/
  ...
  config/       # 厂商预设
  engines/      # 演示引擎：registry / templates / 20 种渲染器 / FitSvg 自适应坐标系 / 动画自动播放
